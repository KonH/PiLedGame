...
namespace SimpleECS.Core.Entities {
	...
	public struct ComponentCollection<T1, T2, T3> : IEnumerable<ValueTuple<T1, T2, T3>>
		where T1 : class, IComponent
		where T2 : class, IComponent
		where T3 : class, IComponent {
		public sealed class Enumerator : IEnumerator<ValueTuple<T1, T2, T3>> {
			List<Entity> _entities;
			...
			public ValueTuple<T1, T2, T3> Current { get; private set; }
		...
		public IEnumerator<ValueTuple<T1, T2, T3>> GetEnumerator() {
			var iter = _cache.Hold<Enumerator>();
			iter.Init(_entities);
			return iter;
		}
